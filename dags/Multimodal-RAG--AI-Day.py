# dags/warm_models_no_fail_dag.py
from datetime import datetime
from airflow import DAG
from airflow.operators.python import PythonVirtualenvOperator

# === CONFIG (from your working script) ===
...
# Minimal Jinja chat template for Transformers >= 4.44 (text-only)
CHAT_TEMPLATE = (
    "{% for m in messages %}"
    "{% if m['role']=='user' %}USER: "
...
def _warm_models_no_fail():
    # import inside the callable so Airflow can parse this file without the deps present
    import json, requests, urllib3
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

    def safe_post(url, headers, payload, label, timeout=60):
        """POST and swallow any error (log body, never raise)."""
        try:
    ...
    # 1) Warm LLaVA (text-only with chat_template)
    ...
    v = safe_post(vision_url, vision_headers, vision_payload, label="vision")
    if v:
        try:
    ...
    embed_payload = {"model": EMBED_MODEL, "input": ["שלום עולם", "Hello world"], "input_type": "passage"}
